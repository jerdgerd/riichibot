...
class PerformanceAnalyzer:
    ...
    def add_game_result(
        self, result: Dict[str, Any], player_stats: List[Dict[str, Any]]
    ):
    ...
    def calculate_average_scores(
        ...
        for i in range(len(self.game_results)):
            ...
            for player_idx in range(4):
                avg_score = (
                    player_totals[player_idx] / total_games
                    if total_games > 0
                    else 25000
                )
    ...
    def analyze_learning_progress(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "win_rate_improvement": [
                late - early for early, late in zip(early_win_rates, late_win_rates)
            ],
            "early_avg_scores": early_avg_scores,
            "late_avg_scores": late_avg_scores,
            "score_improvement": [
                late - early for early, late in zip(early_avg_scores, late_avg_scores)
            ],
        }
    ...
    def generate_report(self) -> str:
        ...
        if "error" not in progress:
            ...
            for i in range(4):
                ...
                report.append(
                    f"  Player {i+1}: {early:.3f} → {late:.3f} ({improvement:+.3f})"
                )
