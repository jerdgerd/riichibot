...
def main():
    parser = argparse.ArgumentParser(
        description="Enhanced training for neural network Mahjong players"
    )
    parser.add_argument(
        "--config",
        type=str,
        choices=["default", "fast", "deep"],
        default="default",
        help="Training configuration preset",
    )
    parser.add_argument(
        "--config-file", type=str, help="Path to custom configuration JSON file"
    )
    parser.add_argument(
        "--save-dir",
        type=str,
    ...
    parser.add_argument(
        "--resume", action="store_true", help="Resume training from existing models"
    )
    ...
    trainer = EnhancedTrainingManager(
        config=config, save_dir=args.save_dir, experiment_name=args.experiment_name
    )
