...
from ai.config import DEEP_TRAINING_CONFIG, DEFAULT_CONFIG, FAST_TRAINING_CONFIG
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["demo", "fast", "standard", "deep"],
        default="demo",
        help="Training mode",
    )
    parser.add_argument(
    ...
    experiment_name = (
        f"quickstart_{args.mode}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )

    # Create trainer
    trainer = EnhancedTrainingManager(
        config=config, save_dir=args.save_dir, experiment_name=experiment_name
    )
    ...
    try:
        ...
        if validation["all_models_present"] and all(validation["models_loadable"]):
            ...
            print("\nNext steps:")
            print(
                f"  1. Play against AI: python src/ai/ai_game.py --model-dir {args.save_dir}"
            )
            print(
                f"  2. Analyze models: python src/ai/analyze_models.py --model-dir {args.save_dir} --all"
            )
