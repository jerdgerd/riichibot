...
def compare_models(model_dir1: str, model_dir2: str, num_games: int = 100) -> Dict[str, Any]:
...
def create_tournament_bracket(model_dirs: List[str], num_games_per_match: int = 50) -> Dict[str, Any]:
...
def visualize_training_progress(model_dir: str, save_path: str = None):
    ...
    for i in range(4):
        ...
        if len(training_losses) > 10:
            smoothed_losses = np.convolve(training_losses[:, i], np.ones(10)/10, mode='valid')
    ...
    if len(win_rates) >= 100:
        ...
        bars = ax4.bar(players, recent_win_rates, color=colors, alpha=0.7, edgecolor='black')
        ax4.set_title('Final Performance (Last 100 Games)', fontsize=14, fontweight='bold')
...
def benchmark_model_speed(model_dir: str, num_decisions: int = 1000) -> Dict[str, float]:
    ...
    for i, player in enumerate(players):
        ...
        for _ in range(num_decisions):
            ...
            action, kwargs = player.choose_action(game_state, player_hand, valid_actions)
...
def validate_model_consistency(model_dir: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if len(players) != 4:
            ...
            validation_results['errors'].append(f"Expected 4 models, found {len(players)}")
        ...
        for i, player in enumerate(players):
            try:
                ...
                if os.path.exists(model_path):
                    ...
                    dummy_hand = {'concealed_tiles': ['1sou'], 'melds': [], 'is_tenpai': False, 'can_riichi': False}
                    dummy_game_state = {
                        'current_player': 0, 'dealer': 0, 'round_number': 1,
                        'wall_tiles_remaining': 70, 'dora_indicators': ['1pin'],
                        'players': [{'name': 'Test', 'score': 25000, 'hand_size': 13, 'melds': 0, 
                                   'discards': [], 'is_riichi': False, 'is_tenpai': False, 'is_dealer': True}] * 4
                    }
                    
                    # Test action selection
                    action, kwargs = player.choose_action(dummy_game_state, dummy_hand, ['discard', 'pass'])
                    
                else:
                    validation_results['models_loadable'].append(False)
                    validation_results['errors'].append(f"Model file missing: player_{i+1}_model.pth")
                    
            except Exception as e:
                validation_results['models_loadable'].append(False)
                validation_results['errors'].append(f"Error testing player {i+1}: {str(e)}")
