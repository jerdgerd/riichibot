...
class NeuralPlayer(Player):
    ...
    def encode_game_state(self, game_state: Dict[str, Any], 
                        player_hand: Dict[str, Any]) -> torch.Tensor:
    ...
    def choose_action(self, game_state: Dict[str, Any], 
                    ...
                    valid_actions: List[str]) -> Tuple[str, Dict[str, Any]]:
        ...
        if random.random() < self.epsilon:
            ...
            action, kwargs = self._select_best_action(action_values, valid_actions, player_hand)
        
        self.last_action = (action, kwargs)
        return action, kwargs

    def _select_best_action(self, action_values: Dict[str, torch.Tensor], 
                        valid_actions: List[str], 
                        player_hand: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
    ...
    def _get_random_action_kwargs(self, action: str, player_hand: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _get_call_kwargs(self, action: str, player_hand: Dict[str, Any]) -> Dict[str, Any]:
        """Get kwargs for call actions"""
        if action == "chii":
            return {"sequence": []}
        return {}

    def remember(self, state: torch.Tensor, action: str, reward: float, 
                next_state: torch.Tensor, done: bool):
        """Store experience in replay buffer"""
        self.memory.append((state, action, reward, next_state, done))

    def give_reward(self, reward: float, next_state: torch.Tensor = None, done: bool = False):
        """Give reward for the last action taken"""
        if self.last_state is not None and self.last_action is not None:
            if next_state is None:
                next_state = torch.zeros_like(self.last_state)
            
            self.remember(self.last_state, self.last_action[0], reward, next_state, done)
    ...
    def replay_training(self):
        ...
        total_loss = torch.tensor(0.0, device=self.device, requires_grad=True)  # Fix: Initialize as tensor
        
        for head_name in current_q_values.keys():
            # Create action mask for this head
            action_mask = torch.zeros(len(actions), current_q_values[head_name].size(1)).to(self.device)
